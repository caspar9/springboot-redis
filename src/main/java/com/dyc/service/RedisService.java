...
public class RedisService {
    ...
    public boolean expire(String key, long time) throws Exception {
        if (time < 0) {
            throw new Exception("expire time must > 0");
        }
     ...
     * @return 值
     */
    public <T> T get(String key, Class<T> objType) {
     ...
     * @param time  时间(秒)
     */
    public void lSet(String key, Object value, long time) throws Exception {
        redisTemplate.opsForList().rightPush(key, value);
        if (time > 0) {
            expire(key, time);
        }
     ...
     * @return
     */
    public void lSet(String key, List<Object> value, long time) throws Exception {
        redisTemplate.opsForList().rightPushAll(key, value);
        if (time > 0) {
            expire(key, time);
        }
